        ...
        attach: function (context, settings) {
            if (context.location == null) {
                $.each(settings.dexp_portfolio, function (filter, view) {
                    var $grid = $('#' + view, context).not('.page-news-media .dexp-grid-items'), $filter = $('#' + filter, context);
            ...
            $.each(settings.dexp_portfolio, function (filter, view) {
                var $grid = $('#' + view, context).not('.page-news-media .dexp-grid-items'), $filter = $('#' + filter, context), $sort = $grid.parent().find('.portfolio-sort') || null;
                $grid.shuffle({
                    itemSelector: '.node',
                    speed: 500
                });
